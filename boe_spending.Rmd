...
```{r}
boe_spending <- read_csv("data/raw/County_Board_of_Education_-_Spending_Disclosures.csv") %>% clean_names()
```
...
```{r}
baltimore_county_2021 <- read_csv("data/raw/baltimore_county_2021.csv") %>% clean_names()
...
baltimore_county_2022 <- read_csv("data/raw/baltimore_county_2022.csv") %>% clean_names()
...
```{r}
baltimore_city_2021 <- read_csv("data/raw/baltimore_city_2021.csv") %>% clean_names() %>% group_by(vendor) %>% summarize(amount = sum(amount)) %>%
...
### Baltimore City FY 2021

```{r}
baltimore_city_2021 <- read_csv("data/raw/baltimore_city_2021.csv") %>% clean_names() %>%
  mutate(fiscal_year = 2021, agency_name = 'BALTIMORE CITY PUBLIC SCHOOLS')
...
anne_arundel_replacement <- read_csv("data/raw/anne_arundel_replacement.csv") %>% mutate(agency_name = str_to_upper(agency_name), payee_name_clean = str_to_upper(payee_name))
...
montgomery_replacement <- read_csv("data/raw/montgomery_2019.csv") %>% clean_names() %>% mutate(agency_name = str_to_upper(agency_name), payee_name_clean = str_to_upper(payee_name))
...
boe_final <- read_csv("data/processed/boe_final.csv") %>% filter(fiscal_year > 2018)

boe_final <- boe_final %>%
  mutate(payee_name = str_replace_all(payee_name, "\u00A0", " "))

# Load exclusion list if it exists
if (file.exists("data/reference/exclude.csv")) {
  exclude <- read_csv("data/reference/exclude.csv") %>% distinct()
  boe_final <- boe_final %>% left_join(exclude)
  boe_final <- boe_final %>%
...
write_csv(boe_spending_cleaned, "data/processed/boe_spending_cleaned.csv")
```
