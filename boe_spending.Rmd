...
```{r}
baltimore_city_2021 <- read_csv("~/code/boe_spending/baltimore_city_2021.csv") %>% clean_names() %>%
  mutate(fiscal_year = 2021, agency_name = 'BALTIMORE CITY PUBLIC SCHOOLS')
...
boe_final <- read_csv("boe_final.csv") %>% filter(fiscal_year > 2018)

boe_final <- boe_final %>% 
  mutate(payee_name = str_replace_all(payee_name, "\u00A0", " "))

exclude <- read_csv("exclude.csv") %>% distinct()
