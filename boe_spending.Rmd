...
montgomery_replacement <- montgomery_replacement %>% left_join(montgomery_original, by=c('payee_name', 'fiscal_year')) %>% select(-agency_name.y, -amount.y) %>% rename(agency_name = agency_name.x, amount = amount.x)
...
boe_spending_cleaned <- boe_spending_cleaned %>% select(agency_name, fiscal_year, amount, payee_name_refined, payee_zip5, slug, purpose_of_payment_baltimore_county_only) %>% 
  rename(payee_name = payee_name_refined, payee_zip = payee_zip5) %>% mutate(purpose_of_payment_baltimore_county_only = replace_na(purpose_of_payment_baltimore_county_only, ""))
